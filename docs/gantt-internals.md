...
### ↩️ Undo 스냅샷 증분(구조 공유) 전환 (2026-09-23, 사용자 지시)

**문제**: `pushUndoSnapshot()`(= `recalculateSchedules()` 종료 시마다 호출)이 `globalData` **전체를
딥카피**해서 최대 50개까지 들고 있었다. 행 수천 개짜리 프로젝트에서는 스냅샷 하나가 수 MB라,
배경 작업(메일 자동배치 등)으로 재계산이 반복되면 메모리가 계단식으로 불어나고 GC가 자주 돌아
앱 전체가 무거워졌다.

**바꾼 방식** (`js/04e-core-app-undo-redo.js`) — 스택 구조와 스냅샷의 "모양"
(`{ globalData:[{data,...}], colIdx }`)은 **그대로 두고**, 바뀌지 않은 행은 직전에 만든 복사본
객체를 그대로 재사용한다:

- `WeakMap<살아있는 행(Array), 복사본>`에 마지막 복사본을 기억해 두고, 스냅샷 때 행마다
  `_rowCopyMatches()`(셀 값 + 언더스코어 속성 얕은 비교)로 대조 → 같으면 **같은 객체를 공유**,
  다르면 새로 복사하고 캐시를 갱신한다.
- 키가 "행 객체 참조"라 **행 삽입·삭제로 인덱스가 밀려도** 공유가 유지된다(인덱스 기반 diff였다면
  삽입 한 번에 뒤쪽 전체가 변경으로 잡혀 이득이 없다).
- `colIdx`도 직렬화 결과가 같으면 같은 객체를 공유한다.
- `_undoStats = {snapshots, rowsCopied, rowsShared}`로 실제 공유율을 확인할 수 있다(콘솔).

**실측** (2000행 × 40열, 스냅샷 50개 — 3회마다 행 삽입, 나머지는 셀 1개 변경):

| | 예전(전체 딥카피) | 지금(구조 공유) |
|---|---|---|
| 힙 증가 | **28 MB** | **2.6 MB** (최초 1회 전체 복사 포함) |
| 소요 시간 | 570 ms | 196 ms |
| 행 복사/공유 | 100,000 복사 | 2,049 복사 / 98,376 공유 |

**같이 고친 잠재 버그**: `_restoreState()`가 스냅샷의 `obj.data` 배열을 **그대로** `globalData`의
행으로 꽂고 있었다 — 복원 직후의 편집이 스택 안 스냅샷까지 같이 바꿔버린다(같은 스냅샷에서 두 번
복원하면 재현). 이제 `Array.from(obj.data)`로 새 배열을 만들어 복원한다. 구조 공유에서는 이게
안전의 전제 조건이기도 하다.

**이 구조를 유지하려면 지켜야 할 규칙**
1. 스냅샷 복사본(`o.data`, `o._xxx`)은 **만든 뒤 절대 수정하지 않는다**. 여러 스냅샷이 같이 가리킨다.
2. 복원할 때는 **반드시 새 배열/새 객체로 복사**해서 `globalData`/`colIdx`에 넣는다.
3. 행의 언더스코어 속성에 객체·배열을 넣고 **그 내용을 제자리에서 수정**하면 변경을 감지하지 못한다
   (참조가 같으므로). 예전 딥카피도 참조만 담았으므로 동작은 동일하지만, 새 속성을 설계할 때는
   "값 교체(불변)"로 다루는 편이 안전하다.
4. Undo는 여전히 행 객체 참조를 **바꾼다**(복원 시 새 배열). 행 참조를 들고 있는 선택 Set 같은 건
   복원 후 무효가 된다 — 예전부터 동일.
