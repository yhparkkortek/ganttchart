...
- **첫 "이슈 정리" 결과(2026-09-21, 이벤트 1건/군집 1개)**: `/sap-fetch` COM 오류 -2147221020 = 환경 이슈. 이 오류는 "SAP GUI 미실행"과 "실행 중이지만 스크립팅 꺼짐"에서 똑같이 나와 기존 안내(스크립팅 설정)가 전자에서 오안내였음 →
...
## 📥 처리결과 되먹임 (resolved_patch.json) — 2026-09-23 추가

**문제**: 경로가 단방향이었다. `앱 → [내보내기] → digest_*.json → Claude가 읽고 고침 → (끝)`.
Claude는 Drive의 `_resolved.json` / `sap_learning.json`에 접근할 수 없어 **자기가 고친 것을 표시할 방법이 없었고**,
그 결과 이미 고쳐진 건(알람 일괄해제·승인원 저장·팀운영비)이 계속 "미해결"로 상위에 남았다. 큐가 쌓일수록
"뭐를 하고 뭐를 안 했는지" 추적이 불가능해진다.

**해결**: Claude가 처리결과 파일을 쓰고, 리포트가 그걸 읽어 미리보기 후 일괄 반영한다.
**판단은 Claude, 승인 클릭은 사람** — 기존 [해결]/[구현됨] 버튼의 일괄 버전이다.

```
C:\SAP_DMS\SAP이슈esolved_patch.json
{
  "generatedBy": "Claude (Opus 5)", "generatedAt": "...", "source": "digest_20260923.json",
  "issues":   [{ "sig": "<군집 시그니처 그대로>", "note": "고친 내용 한 줄", "commit": "abc1234" }],
  "learning": [{ "key": "<학습 적립 key>", "status": "done|hold|reviewing|new", "note": "...", "commit": "abc1234" }]
}
```

- 반영: 리포트 헤더 **[📥 처리결과 반영]** → 파일 선택 → **미리보기**(대상/현재→변경/사유·커밋) → [✅ 반영하기].
  `issues`는 `_resolved.json`에, `learning`은 `sap_learning.json`에 저장된다(기존 버튼과 동일한 저장소).
- `note`에는 사유, `commit`은 괄호로 붙여 `[abc1234]`로 기록된다 — 나중에 "왜 해결로 표시됐는지"를 추적할 수 있다.
- 미리보기의 `*` 표시 = 현재 조회 범위(기본 3개월)에 없는 항목. 그래도 기록은 남긴다(나중에 재발하면 바로 판정됨).
- 한 파일 최대 500항목(`PATCH_MAX`). 같은 파일을 두 번 반영해도 안전하다(멱등 — "변화 없음"으로 표시됨).
...
| 수집·군집·리포트 생성 | 앱(자동) |
| 내보내기 | 사람(버튼 1회) |
| 진단·수정·**처리결과 파일 작성** | Claude |
| 반영 승인 | 사람(미리보기 후 1클릭) |
| 재발 판정 | 앱(자동 — `resolved.at` / `doneAt` 이후 재발생 시 배지) |
| 삭제 | **없음** — 원본은 Drive 샤드에 남고, 리포트는 기간(기본 3개월) 밖이 되면 자연스럽게 빠진다 |
