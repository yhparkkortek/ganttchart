...
- **범위**: 이 변경은 "AI 문답에서 BOM 열어줘" 경로에만 적용된다. 승인원 표지 생성(`_ganttQaApprovalDraft`),
  사용처/역전개, ZMM009 등 SAP 문서 데이터를 참고해 **AI가 더 넓은 판단**(다른 업무와 연결·요약 등)을
  해야 하는 다른 요청은 손대지 않음 — 순수 "조회해서 보여주기"만 AI 없이 처리.