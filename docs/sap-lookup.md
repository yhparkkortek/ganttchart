...
**ZMM009엔 품목2 컬럼이 없다는 걸 라이브로 확인** — 예전 코드 주석은 "ZMM009 그리드에도
자재내역(KO)/자재내역2(KO) 두 컬럼이 그대로 노출된다"고(2026-09-17, 사용자 스크린샷 기반)
적혀 있었는데, 2026-09-23 실제로 `/sap-zmm009`를 22개 자재로 직접 호출해서 헤더 행을
확인해보니 **MAKTX(자재내역) 하나만 있고 품목2에 해당하는 컬럼이 없었다**(품목2 원본인
"기본 데이터 텍스트"는 MM03의 "추가 데이터" 서브화면에만 있는 데이터라 ZMM009 화면
자체에 없음 — 예전 판단이 틀렸던 것). 그래서:
- **"품목2"/"내역2"를 명시적으로 요청했을 때만** 기존처럼 느린 `fetch_approval_info`(MM03
  자재마다 진입)를 쓴다 — 그 데이터를 얻을 수 있는 유일한 경로라 느려도 어쩔 수 없음.
- **품목2를 요청 안 했으면 자재 수와 무관하게 항상 빠른 ZMM009**(`/sap-zmm009`)로 보낸다 —
  `_ganttQaParseZmm009DescriptionMap`(js/04h)이 ZMM009 원본 텍스트에서 MATNR/MAKTX 두
  컬럼만 뽑아 자재번호→품목 사전으로 만든다.
