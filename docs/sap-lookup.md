...
**🐛✅ [2026-09-23 실사용 버그리포트 → 원인 확인 → 수정 완료] 기간을 연간(1~12)이 아니라
좁혀서(예: "1~6월까지") 조회하면 550203 행을 못 찾던 문제** — 아래 "화면 트리 통째로 덤프"
도구로 실제 화면을 떠서 원인 확정:
- `usr` 영역의 GuiLabel Id가 **`lbl[열,행]`** 좌표 패턴이고(예: `lbl[5,6]`="550203 복리후생비-
  팀운영비(개발)", `lbl[41,6]`~`lbl[97,6]`이 예산금액~잔여예산 5개 값), 부분 기간(1~6월)에는
  **"임시전표" 칸(`lbl[69,6]`)이 값 0이라 빈 문자열로 렌더링**됐다.
- 기존 `_sap_find_team_budget_row`(현재 `_sap_find_team_budget_row_sequential`로 개명)는
  `_sap_dump_fields`로 화면 라벨을 순서대로 모으는데, **빈 텍스트인 라벨은 통째로 건너뛴다**
  (`_sap_dump_fields`의 `if text:` 필터) — 그래서 "550203" 다음 5칸을 순서대로 슬라이싱하면
  중간에 하나가 빠져 4개만 잡히고, `len(values) == 5` 검증에 걸려 "못 찾음"으로 떨어졌다.
  연간(1~12월) 조회는 우연히 임시전표가 0이 아니어서 문제가 없었을 뿐.
- **수정**: Id의 `lbl[열,행]` 좌표를 직접 파싱하는 `_sap_parse_label_grid`/
  `_sap_find_team_budget_row_by_coords`를 1차 경로로 추가 — 헤더 행(예산금액/전기금액/…)에서
  각 컬럼의 열(column) 번호를 찾고, "550203"이 들어간 행(row)에서 그 열들의 값을 좌표로 직접
  읽는다. 빈 칸이 있어도 열 위치가 고정이라 밀리지 않는다. 좌표 패턴이 아닌 화면(향후 다른
  트랜잭션)을 위해 기존 순서 기반 방식은 `_sap_find_team_budget_row_sequential`로 남겨 폴백.
- **실사용 검증**: `/sap-team-budget?team=개발3팀&from=1&to=6` 직접 호출로 확인 — 예산금액
...
**🐛 [2026-09-23 실사용 버그수정] 첫 시도에서 "회계 1" 탭의 가격 필드를 못 찾고 실패** — 탭
안에 다시 중첩된 기간 탭스트립(PPLF)의 서브화면이 탭 전환 직후 바로 안 잡혀서 고정 0.4초
대기로는 부족했다(연속 자동 테스트로 재현 확인). `_select_tab_with_retry`와 같은 패턴으로
필드 읽기 자체를 최대 6회(0.5초 간격) 재시도하도록 고침.

### 📦 "품목 내역" 다중조회를 ZMM009로 전환 + SAP 작업 중단 명령 (2026-09-23, 실사용 제보)

**제보**: "22개 자재 품목 내역(2 포함) 조회해줘"가 MM03을 자재마다 순회하느라 너무 오래
걸리는데, 중단할 방법도 없었다. 사용자 지적: "MM03=단수, ZMM009=복수 사용하면 되는데
(기본으로 ZMM009 써도 됨)".

**ZMM009의 품목2 컬럼(ZTEXT)은 존재하지만 계정별 저장 레이아웃에 숨겨져 있을 수 있음** —
처음엔 "ZMM009 그리드엔 품목2 컬럼이 아예 없다"고 결론 냈었다(2026-09-23, `/sap-zmm009`를
22개 자재로 직접 호출해서 헤더 행에 MAKTX만 있고 품목2에 해당하는 게 없는 걸 확인). 그런데
**사용자가 직접 SAP에서 "레이아웃 변경"(헤더 오른쪽클릭 → 표시)을 열어 "자재내역2(KO)"라는
이름의 숨겨진 열을 찾아 표시로 옮겨줌** — 기술 필드명은 `ZTEXT`(헤더에서 MAKTX 바로 다음).
즉 **컬럼 자체는 항상 존재하지만, SAP ALV는 컬럼 표시 여부를 사용자별 저장 레이아웃으로
관리해서, 그 레이아웃에 아직 추가 안 된 계정은 `ColumnOrder`(그리고 이 코드베이스가 읽는
`GetCellValue` 순회)에도 안 잡힌다** — 처음 판단(컬럼 없음)은 "이 계정 레이아웃엔 없었다"의
착각이었다. 그래서 최종 구현은 매번 헤더에 ZTEXT가 실제로 있는지 확인하는 방식으로 바꿈:
- **항상 먼저 빠른 ZMM009로 조회**(`/sap-zmm009`) — `_ganttQaParseZmm009DescriptionMap`(js/04h)이
  MATNR/MAKTX/(있으면)ZTEXT를 뽑아 `{matnr: {maktx, ztext}}` + `hasZtext` 플래그로 반환.
- **품목2를 요청 안 했으면**(`wantsSub=false`) ZMM009 결과의 MAKTX만으로 바로 끝.
- **품목2를 요청했는데 이번 응답에 ZTEXT가 있으면**(그 SAP 계정 레이아웃에 이미 추가돼
  있으면) 그것도 ZMM009 한 번으로 끝 — MM03을 전혀 안 들른다.
- **품목2를 요청했는데 ZTEXT가 없으면**(레이아웃에 아직 없는 계정) 그때만 느린
  `fetch_approval_info`(MM03 자재마다 진입)로 폴백하고, 답변 끝에 "ZMM009 레이아웃에
  '자재내역2(KO)' 열을 추가하고 저장해두면 다음부터 빨라진다"는 안내를 덧붙인다 —
  **그 열을 레이아웃에 추가+저장하면, 사람이 하든 이 앱이 자동으로 하든 이후 모든 ZMM009
  조회가 코드 변경 없이 그냥 빨라진다**(SAP ALV 레이아웃은 계정별로 저장되는 설정이라).
- 판정은 `_ganttQaExtractMaterialInfoRequest`가 `/(품목\s*2|내역\s*2|자재내역2)/`로 원문에서
  직접 감지(`wantsSub`) — 새 SAP 자동화 없이 기존 `/sap-zmm009`를 그대로 재사용.

**SAP 작업 중단 명령(`/sap-cancel`, 2026-09-23 신규)**: 위처럼 어쩔 수 없이 느린 경로(또는
예상보다 오래 걸리는 다른 SAP 조회)를 사람이 "그만"/"중단"/"멈춰"/"실행 중단해줘"라고
말하면 멈출 수 있다.
- **백엔드**: `kortek_backend.py`의 `_run_sap_bridge`를 `subprocess.run`(블로킹) 대신
  `Popen`으로 바꿔 진행 중인 프로세스 핸들을 전역(`_SAP_PROC_STATE`)에 보관 — 새
  `POST /sap-cancel`이 다른 요청 스레드에서 그 핸들을 `kill()`한다. 이게 동작하려면 Flask가
  동시에 두 요청을 처리해야 해서 `app.run(..., threaded=True)`로 바꿈(기존엔 미지정=단일
  스레드였음). 외부에서 kill되면 원래 요청 스레드의 `proc.communicate()`가
  `TimeoutExpired` 없이 정상적으로(빈 출력과 함께) 리턴하고, `_SAP_PROC_STATE['cancelled']`
  플래그로 "타임아웃"과 "사용자 중단"을 구분해 다른 메시지를 돌려준다.
- **프런트엔드**: `js/04h`의 `sendGanttQaMessage` 맨 앞(`_ganttQaSending` 재진입 가드보다도
  먼저!)에 새 로컬 명령을 둠 — 그래야 다른 SAP 로컬 명령이 `await` 중이어도(그 블록들은
  `_ganttQaSending`을 안 건드리므로 JS 이벤트 루프가 이 새 입력을 바로 처리할 수 있음)
  "그만"이 먹힌다. **⚠️ 진행 중인 요청이 있으면(마지막 히스토리가 pending 말풍선) 새
  메시지를 push하지 않고 그 말풍선을 직접 갱신한다** — 새로 push하면, 잠시 뒤 원래 요청이
  응답을 받고 실행하는 `history.pop()`(배열의 "마지막 항목"을 지우는 기존 관례)이 방금 push한
  메시지를 엉뚱하게 지워버려 원래 pending 말풍선이 "⏳"인 채로 영원히 남는 버그가 생긴다 —
  이건 이 코드베이스의 `push(pending)...await...pop()+push(final)` 패턴 전체가 안고 있는
  구조적 취약점이라, 앞으로 이 패턴 위에 무언가를 끼워 넣을 때마다 같은 문제를 주의할 것.
