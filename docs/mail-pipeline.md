...
- **🐛🐛 정정(2026-09-22 오후, "SAP 연결 말고 달라진 게 없는데 AI가 전부 막힌다")**: `limit: 20`(generate_content_free_tier_requests)은 분당(RPM)이 아니라 **무료 등급 "하루 20회"(RPD, 모델별)** 였다. 구글은 RPM/RPD를 같은 metric 이름으로 보내고 일일 한도에도 "Please retry in 29s"를 붙인다 — 이를 RPM으로 단정한 안내("30초면 풀림")와 34초 쿨다운 때문에 한도가 끝난 날에도 34초마다 Gemini 전 후보+Groq+Mistral을 두드렸다. 리셋은 태평양 자정(한국 16:00/17:00). 수정 4가지:
  ...
  4. 사용량 원장 `gantt_ai_usage_v1`(태평양 날짜별 7일): 모델별 성공/실패, 기능별 요청 수(`opts.feature` 없으면 콜스택의 "함수명(파일)" 자동 추출). ⚙️ AI 분석 설정의 쿨다운 항목 아래 "📊 오늘 AI 사용량"과 일일 한도 오류 메시지에 요약 표시. **다음에 한도가 빨리 소진되면 코드 재조사 전에 이 원장부터 볼 것.**- 같은 날 `js/14a`에 있던 **자동전환 후보에서 유료(`tier:'paid'`) 모델 제외** 수정도 함께 적용됨(실사용 확인: `gemini-3.1-pro`가 무료 등급 한도 "limit: 0"으로 거부됨 — 자동전환 후보에 남겨봐야 매번 헛되이 한 번 더 실패만 함).
