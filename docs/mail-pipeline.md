...
### 🧹 저장소 선제 점검 — 가득 차고 나서가 아니라 차기 전에 줄인다 (2026-09-23, Phase 10 학습 자료 반영)

`digest_20260923`에서 `storage_full` 5건 + `storage_report` 1건이 연달아 기록됐고, 그때 저장소는
**5116KB로 사실상 한도(≈5MB)에 붙어** 있었다(`ms_pending_queue` 2394KB + `gantt_task_inbox` 1624KB).
기존 구조는 **저장이 실패한 뒤에야**(`_storageDoctorAuto`) 캐시를 비워서, 그 사이 보관함 저장이
조용히 실패하는 구간이 생겼다.

- `window._storageDoctorCheck({threshold})` (`js/34`) — 사용량을 재서 임계치(기본 **80%**, 기준 5MB)를
  넘으면 ① 캐시류(`plan('cache')`)를 먼저 자동 정리하고 ② 그래도 높으면 **하루 한 번만** 안내한다
  (`gantt_storage_notice_day`). 반복 토스트로 사람을 괴롭히지 않는 게 중요하다.
- 실행 시점: 로드 2분 뒤 1회 + 30분 주기(초기 로드·드라이브 연동과 겹치지 않게).
- `probeFree()`는 **쓰지 않는다** — 큰 문자열을 실제로 써 보는 방식이라 점검이 저장소를 또 흔든다.
  대신 통상 한도 5MB를 기준치로 삼는다(`SD_BUDGET`).
...
- 메일 자동틱(`_autoMailFetchTick`)도 같은 문제가 있었는데(메일마다 1건씩, fire-and-forget이라
  묶기 어려움), `_msAutoRegisterToProject` 안에 **코얼레서**를 두어 해결했다 — 현재 프로젝트 배치는
  `_msScheduleCurrentProjectFinalize()`로 10초(`window.MS_FINALIZE_COALESCE_MS`) 창을 열고, 그 안에 들어온
  배치들을 **재계산 1회 + 저장 1회**로 끝낸다(타이머를 리셋하지 않는 비리셋 방식 — 리셋하면
  4초 간격 호출 때문에 영영 저장되지 않는다). 예약된 마무리는 탭이 숨겨질 때와 일괄전송 직후에
  `_msFlushCurrentProjectFinalize()`로 즉시 비운다. **사람이 직접 누른 단건 전송은 `{coalesce:false}`**로
  예전처럼 즉시 저장한다(버튼을 누른 사람을 10초 기다리게 하지 않음).
...
그런데 `loadFromDrive()`는 **uid 기준 병합**(로컬 우선, 드라이브에만 있으면 살림)이라 **삭제를 표현할 수가 없다**
...
**삭제 묘비(tombstone)** — 병합 방식에서 삭제를 표현하는 유일한 방법(2026-09-23 추가):

- 저장소: `localStorage['gantt_task_inbox_deleted']` = `[{uid, at}]`, 드라이브 파일엔 `{inbox, deleted, savedAt}`의 `deleted`로 같이 올라간다.
- 기록 시점: `remove()`(단건 🗑), 묶음 삭제(`_tiBulkDeleteFiltered`), '처리됨 자동삭제' 모드의 `setStatus`.
  **예외**: `_shrinkForQuota`(용량 초과 응급 정리)는 묘비를 남기지 않는다 — 그건 이 PC의 사정일 뿐 "지우자"는 의사가 아니라, 다른 기기 데이터까지 지우면 안 된다.
- `loadFromDrive()`는 로컬+원격 묘비를 합쳐(`_mergeTombs`) 저장하고, 병합 결과에서 그 uid를 제외한다 — 다른 기기에 남아있던 항목도 이 시점에 정리된다.
- 되돌리기(↩)는 `unmarkDeleted()`로 **묘비를 지우는 것**으로 표현한다(그리고 즉시 업로드).
- 수명: 90일 또는 1000건(오래된 것부터) — `TOMB_MAX_DAYS` / `TOMB_MAX`. 저장소 닥터 레지스트리(`js/34`)에도 등록돼 있다.

> ⚠️ 묘비를 손으로 지우면(저장소 정리 포함) 다른 기기에 그 항목이 남아있을 때 되살아날 수 있다. 그래서 cache가 아니라 log로 분류했다.
