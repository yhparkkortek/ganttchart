...
#### 헤더의 보조 액션 버튼(새로고침·내보내기·프롬프트편집 등) — 테두리 없이, hover는 배경만 (2026-09-22 추가, 22일 정정)
**닫기(✕) 버튼뿐 아니라 헤더에 두는 다른 버튼도 전부 hover 필수** — 이 규칙이 "닫기 버튼 스타일"
절에만 있고 일반화돼 있지 않아서, 새 모달 헤더 버튼에 hover가 누락되는 사고가 반복됐다(이슈
리포트 모달의 🔄새로고침/📤내보내기 버튼, 2026-09-22 발견). **헤더 버튼은 하단 액션 버튼(첨부/
전송 등, 아래 절)과 톤이 다르다 — `border:none`(테두리 자체가 없음), 배경은 헤더 배경과 거의
같은 톤으로 "얹혀있는" 느낌을 주고 hover 시 배경만 한 단계 진해진다(테두리가 없으니 테두리
색은 바꿀 게 없다)**: 실제 코드(AI 문답 🔇/📝프롬프트/🕒이력 버튼, `js/04h`/`04f`/`14a`)의
검증된 패턴은 `style="background:#e8f4fd; border:none; ..." onmouseover="this.style.background='#cfe6fa';" onmouseout="this.style.background='#e8f4fd';"`.
살구색 헤더는 `#fff8e6` → hover `#ffefc0`, 하늘색(AI) 헤더는 `#e8f4fd` → hover `#cfe6fa` — 새
헤더 버튼도 이 두 톤 중 헤더 색에 맞는 쪽을 그대로 재사용할 것(테두리 넣지 말 것, 새 색 만들지 말 것).
(하단 첨부/전송/삭제처럼 **헤더가 아닌 몸통의 큼직한 액션 버튼**은 반대로 `border:1px solid <색>`이
있고 hover 때 배경과 테두리 색을 함께 진하게 하는 별도 패턴이다 — 초록(확인) `#e6f6ea`/`#a8dab8` →
`#c9ecd3`/`#7cc494`, 빨강(삭제) `#fdecec`/`#f0b8b8` → `#f8d4d4`/`#e59a9a`. 헤더 버튼에 이 테두리형
패턴을 섞어 쓰지 말 것 — 2026-09-22에 실제로 섞어 썼다가 사용자 지적으로 수정한 사례.)
