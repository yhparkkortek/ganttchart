...
${docsJson}`;
    ...
    window.sendGanttQaMessage = async function() {
        ...
        //    강제로 멈춘다. 아래 `_ganttQaSending` 재진입 가드보다 **먼저** 체크해야 한다 —
        //    지금 진행 중인 SAP 로컬 명령들(품목내역/BOM/화면덤프 등)은 그 가드를 안 쓰므로
        //    실제로는 안 걸리지만, 혹시 걸리는 경로가 있어도 "중단해줘" 자체는 항상 먹혀야
        //    의미가 있다. kortek_backend.py의 POST /sap-cancel이 지금 돌고 있는
        //    32비트 서브프로세스(sap_bridge_32.py)를 강제종료 — 그러면 그 요청을 보낸 원래
        //    로컬 명령의 fetch()도 곧 "⏹ 사용자 요청으로 중단되었습니다" 응답을 받고 알아서
        //    끝난다(별도 AbortController 불필요).
        //    ⚠️ 진행 중인 요청이 있으면(=마지막 히스토리가 pending 말풍선) **새 메시지를 push하지
        ...
        if (_sapCancelRe.test(question)) {
            ...
            if (_pendingMsg) {
                ...
                window._renderGanttQaMessages();
                try { await fetch('http://127.0.0.1:5000/sap-cancel', { method: 'POST' }); } catch (e) { /* 원래 요청이 어차피 자체적으로 실패 응답을 받고 끝남 */ }
                input.focus();
                return;
            }
            ...
            let _cancelReply;
            try {
                const res = await fetch('http://127.0.0.1:5000/sap-cancel', { method: 'POST' });
                const data = await res.json();
                _cancelReply = data.cancelled
                    ? window._t('⏹ 진행 중이던 SAP 작업을 중단했습니다.', '⏹ Stopped the ongoing SAP operation.')
                    : window._t('중단할 작업이 없습니다.', 'Nothing is currently running to stop.');
            } catch (e) {
                _cancelReply = '⚠️ ' + window._t('중단 요청 실패: ', 'Failed to send stop request: ') + (e && e.message ? e.message : e);
            }
            window._ganttQaHistory.push({ role: 'ai', text: _cancelReply });
            window._renderGanttQaMessages();
            input.focus();
            return;
        }
    ...
    window._ganttQaTryHandleAlarmFilterCommand = function(question) {
        ...
        // 🐛 [2026-09-22 실사용 버그수정] 이 패턴에 ^...$ 전체일치 앵커가 빠져 있어서(위 주석은
        //    "전체 일치만 허용"이라고 돼 있었지만 실제로는 부분일치였음), "g310 핀셋 해제해줘"처럼
        //    특정 업무 하나의 알람을 끄려는 요청까지 "핀셋 해제"라는 부분 문자열만 보고 이 필터
        //    토글 명령으로 잘못 가로채 "✅ 알람 필터는 이미 꺼져 있습니다"만 답하고 정작 그 업무의
        //    알람은 전혀 안 건드리는 사고가 있었다(실사용 제보). \S*+뒤에 붙는 "해줘/주세요" 같은
        //    흔한 꼬리말은 여전히 허용하되, 전체 메시지가 "알람 끄기류 문구 하나"일 때만 매칭하도록
        //    ^...$로 좁힘 — "g310"처럼 앞에 다른 내용이 붙으면 이제 매칭 안 되고 일반 AI 경로로 감.
        var _offTail = '(?:해\\s*줘|해\\s*주세요|주세요|줘|부탁\\S*)?\\s*[.!?~]*';
        var offPattern = new RegExp(
            '^\\s*(?:' + _aw + ')\\s*(?:해제|끄기|끄|off\\b)\\S*' + _offTail + '\\s*$' +
            '|^\\s*alarm\\s*(?:filter\\s*)?(?:off|해제|끄기)\\S*' + _offTail + '\\s*$' +
            '|^\\s*전체\\s*(?:보기|표시|보여줘)\\s*$'
        , 'i');
    ...
    window._ganttQaExtractMaterialInfoRequest = function(question) {
        ...
        // 🆕 [2026-09-23 신규, 사용자 요청] "품목2"/"내역2"까지 명시적으로 물어봤는지 —
        // 이게 있어야만 아래 호출부가 MM03(느리지만 품목2까지 되는 유일한 경로)을 쓰고,
        // 없으면 ZMM009(빠르지만 품목2는 없음)로 보낸다. 근거는 아래 호출부 주석 참고.
        var wantsSub = /(품목\s*2|내역\s*2|자재내역2)/.test(text);
    ...
    window.openGanttQaModal = function() {
        ...
        if (!modal) {
            ...
            <div id="gantt-qa-box" onclick="event.stopPropagation()" ondragover="window._ganttQaHandleDragOver(event)" ondragleave="window._ganttQaHandleDragLeave(event)" ondrop="window._ganttQaHandleDrop(event)" style="pointer-events:all; position:fixed; background:#ffffff; border-radius:10px; width:var(--modal-w-md); max-width:92vw; max-height:80vh; display:flex; flex-direction:column; box-shadow:0 8px 32px rgba(0,0,0,0.22); top:50%; left:50%; transform:translate(-50%,-50%); resize:both; overflow:hidden; min-width:320px; min-height:380px;">
                ...
                <!-- 🧹 [2026-09-21 UI 정리, 사용자 요청] 위쪽 설명 2줄(데이터 근거·답변 지연 안내)은 삭제. 질문 대상 / 자주 쓰는 질문 두 줄은 입력창 바로 위로 옮겼고,
                     오른쪽 열에 [분류 드롭다운(라벨 없이)] 위 · [대화삭제] 아래를 같은 폭으로 정렬했다. 예전의 커다란 [대화 삭제] 버튼은 없앤 만큼 입력창이 넓어졌다.
                     • 질문 대상: "질문 대상"만 고르면 왕복 없이 답하는 가벼운 조회 경로와 별개로, 진짜로 그 프로젝트를 열어서(=현재 프로젝트로 전환) 100% 동일한
                       조건으로 묻고 싶을 때를 위한 [🔓 열기] 지름길 — executeLoadFile 재사용. 다른 프로젝트가 선택된 동안에만 드롭다운 오른쪽 끝에 보임.
                     • 자주 쓰는 질문: 대화 중에도 계속 골라 쓸 수 있게 항상 보이는 드롭다운. 선택하면 입력창에 채워짐(바로 전송 안 됨) — _ganttQaFillQuestion 재사용.
                     • 분류(#gantt-qa-route-col): 질문 라우터(js/33 ensureChips)가 이 칸 안에 선택 상자를 만든다(라벨은 만들지 않음, 설명은 title). -->
                <div style="border-top:1px solid #d0dde8; background:#f6f8fa;">
                <div style="padding:8px 14px 0; display:flex; gap:8px; align-items:stretch;">
                    <div style="flex:1; min-width:0; display:flex; flex-direction:column; gap:6px;">
                        <div style="display:flex; align-items:center; gap:6px; min-width:0;">
                            <label id="gantt-qa-target-label" for="gantt-qa-target-project" style="flex:0 0 96px; font-size:10.5px; color:#888; white-space:nowrap;">${_qEn ? '📂 Target' : '📂 질문 대상'}</label>
