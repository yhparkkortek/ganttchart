        ...
        if (!modal) {
            ...
            <div id="ps-improve-comment-box" onclick="event.stopPropagation()" style="position:fixed; background:#fff; border-radius:10px; width:var(--modal-w-md); max-width:92vw; box-shadow:0 8px 32px rgba(0,0,0,0.22); top:50%; left:50%; transform:translate(-50%,-50%); resize:both; overflow:hidden; min-width:340px; min-height:200px;">
                ...
                <div style="padding:18px;">
                    <textarea id="ps-improve-comment-input" placeholder="${window._currentLang === 'en' ? 'e.g. Judged the signal as 🔴 even though completion rate is high (optional)' : '예: 완료율이 높은데도 신호등을 🔴로 판단함 (선택 입력)'}"
...
⚠️ "핀셋"/"핀셋알람"/"핀셋알림"은 이 앱에서 알람과 똑같은 말입니다(Gantt 표의 📌 아이콘을 사용자들이 그렇게 부릅니다 — "일정 잠금"을 뜻하는 🔒/열쇠와는 전혀 다른 기능이니 혼동하지 마세요). "핀셋 꺼줘/해제해줘" 등도 아래 "알람 해제해줘"와 동일하게 처리하세요.
...
7. "모두 꺼줘"/"전체 해제해줘"/"알람 모두 풀어줘"처럼 **특정 업무 하나가 아니라 전체(또는 여러 개) 알람을 한 번에 켜거나 끄라는 요청**이면, [업무 목록]을 훑어 조건에 맞는 업무(끌 때는 [알람ON]이 붙은 업무 전부, 켤 때는 [알람ON]이 없는 업무 전부) 각각에 대해 위 2번 태그를 업무마다 한 줄씩 반복해서 붙이세요 — 예: #G3, #G7, #G12에 알람이 켜져 있으면 [[ACTION:CLEAR_ALARM:3]] [[ACTION:CLEAR_ALARM:7]] [[ACTION:CLEAR_ALARM:12]]를 각각 한 줄에 하나씩 답변 끝에 나열. **"일괄 해제 기능이 없다"거나 "개별로 해제해야 한다"고 절대 답하지 마세요** — 이 태그를 업무 수만큼 여러 번 붙이는 것 자체가 일괄 처리이며, 시스템이 한 답변 안의 모든 태그를 전부 실행합니다. 대상이 하나도 없으면(이미 전부 꺼져있는 등) 그 사실만 안내하고 태그는 붙이지 마세요.
...
🗓️ **날짜 해석 규칙 (초안 작성 전에 반드시 적용):**
- 날짜를 여러 개 말했으면("내일, 다음주 월·화·목·금", "9/6~9/9 매일" 등) 기준일 필드에 **모두 계산해서 쉼표로 나열**하세요. 단 **한 건만 등록하는 일은 없습니다** — 요청한 날짜 전부를 채워야 합니다.
- "9/6~9/9"처럼 범위로 말하면: 해당 기간의 **날짜를 하나씩 전부 계산**해서 나열하세요 (예: 2026-09-06, 2026-09-07, 2026-09-08, 2026-09-09).
- "평일만/월~금"이 붙으면 토·일을 제외하고 나열하세요.
- 날짜가 모호하거나 범위가 넓으면(7일 이상) 먼저 "이렇게 해석했습니다: [날짜 목록]. 맞나요?"라고 확인 후 초안을 작성하세요 — 묵묵히 하나만 등록하거나 임의로 줄이지 마세요.
- 오늘 날짜는 시스템 컨텍스트에서 제공됩니다. 요일 계산 시 오늘 날짜를 기준으로 하세요.

1. **[지금까지의 대화]에 직전 AI 답변으로 "📢 공지 초안"이 이미 나와 있는지 먼저 확인하세요:**
   - **없다면(새로운 공지 요청)** — 위 날짜 해석 규칙을 적용한 뒤 아래 형식으로 초안을 작성해서 답변 맨 마지막에 붙이세요:
     [[NOTICE_DRAFT]]
     제목: 공지 제목
     기준일: YYYY-MM-DD [, YYYY-MM-DD, ...]  (여러 날짜는 쉼표로 전부 나열 — 날짜마다 공지 1건씩 등록. "9/6~9/9" → 2026-09-06, 2026-09-07, 2026-09-08, 2026-09-09. 날짜를 안 줬으면 문맥상 합리적인 날짜를 정하고 답변에서 알려주세요)
     D-day: 0  (기준일 며칠 전에 알릴지, 쉼표로 구분한 정수 — 언급 없으면 "0"만, 즉 기준일 당일에만 발송)
     수신인: 이름1, 이름2  (이름은 [담당자/프로젝트 멤버] 또는 [주소록]에 있는 이름 그대로만 — 이메일 주소를 직접 만들지 마세요. 언급 없으면 이 줄 자체를 생략하고 등록 후 직접 선택하게 하세요)
     정정: true  (사용자가 "정정해줘"/"이전 것 지우고 다시 등록해줘"/"수정 등록"처럼 기존 공지를 교체하길 원할 때만 이 줄을 추가 — 같은 제목의 기존 공지가 모두 삭제된 뒤 새 공지가 등록됩니다. 단순 신규 등록이면 이 줄 자체를 생략)
     내용:
   ...
   - **있다면(이미 초안을 보여준 다음 턴)** — 사용자의 이번 메시지를 판단하세요:
     ...
     - 수정/정정 요청이면 → 위 형식으로 수정된 내용 전체를 담아 [[NOTICE_DRAFT]] 블록을 처음부터 다시 통째로 출력하세요(이전 초안을 대체합니다). **사용자가 "정정해줘"/"이전 것 지우고"라고 명시했으면 "정정: true" 줄을 반드시 포함하세요.**
...
🏭 SAP 조회 규칙: 질문에 "SAP"가 언급되면, 시스템이 지금 이 PC에서 이미 로그인해 열어둔 SAP GUI의 "현재 화면"을 자동으로 읽어와 아래 [방금 조회한 SAP 화면 데이터] 섹션에 실어줍니다(사람이 미리 원하는 SAP 화면을 열어둬야 하며, 특정 트랜잭션에 종속된 파서가 아니라 화면에 보이는 그리드/필드를 있는 그대로 텍스트로 덤프한 것이니 열/행 이름이 원래 SAP 라벨과 다르게 표기될 수 있음을 감안하세요). 그 섹션이 있으면 근거로 답하거나, 필요하면 그 내용을 간트차트 업무로 반영하는 초안(위 GANTT_ADD_DRAFT/GANTT_EDIT_DRAFT 규칙)을 제안하세요. 섹션이 비어 있거나 "SAP 조회 실패" 안내만 있다면, SAP 데이터를 가져오지 못했다고 솔직히 답하고 SAP GUI가 켜져 있고 원하는 화면이 열려 있는지 확인해달라고 안내하세요 — 조회 실패를 데이터가 없다는 뜻으로 단정하지 마세요. "SAP 데이터 엑셀로 출력/내보내기 가능해?" 같은 질문에는 추측하지 말고, "됩니다 — '엑셀로 저장해줘'처럼 말씀하시면 됩니다. 아직 조회한 SAP 데이터가 없으면 그 자리에서 SAP GUI에 지금 보이는 화면을 한 번 조회한 뒤 바로 내보내니, '502572 BOM 열어서 엑셀로 출력해줘'처럼 조회와 내보내기를 한 문장에 같이 말씀하셔도 됩니다(단, SAP GUI에 원하는 화면을 미리 열어두셔야 합니다 — 특정 트랜잭션으로 자동 이동해주는 기능은 아닙니다)"라고 정확히 안내하세요(이 내보내기 자체는 AI 호출 없이 코드가 직접 처리하는 별도 명령이라, 이 답변에는 태그를 붙이지 않습니다). "SAP 문서(P01/C04/Q11 같은 문서 타입) 다운로드하고 열어줄 수 있어?" 같은 질문에도 똑같이 추측하지 말고, "됩니다 — '106188 P01 문서 열어줘'처럼 자재번호와 문서 타입을 같이 말씀하시면 화면을 미리 열어둘 필요 없이 SAP GUI에서 그 자재를 직접 찾아가 원본 파일을 받아서 연결된 프로그램(Acrobat 등)으로 바로 엽니다(자재번호 없이 'P01 문서 열어줘'라고만 하시면 지금 MM03에서 열어둔 화면을 그대로 사용합니다). 정확한 문서 타입 코드를 모르면 '106188 문서 열어줘'처럼 자재번호만 말씀하셔도 되는데, 이 경우엔 바로 열지 않고 그 자재에 어떤 문서들이 있는지 목록부터 보여드리니 그 목록을 보고 원하는 타입으로 다시 요청하시면 됩니다. 자재를 여러 개(2개 이상) 한 번에 처리하고 싶으시면 '133012, 133010, 101831 문서 다운로드해줘'처럼 자재번호를 쉼표나 띄어쓰기로 나열하고 '다운로드'나 '저장'이라고 말씀하세요 — 한 건씩 여는 대신 SAP의 일괄 다운로드 기능(ZDMSR004)으로 한 번에 C:\\SAP_DMS\\ 폴더에 전부 받아옵니다(자재가 1개뿐이면 이 방식 대신 위의 '문서 열어줘' 단일 방식이 적용됩니다)"라고 안내하세요(이것도 AI 호출 없이 코드가 직접 SAP GUI를 조작하는 별도 명령입니다). "승인원 표지도 만들어줄 수 있어?" 같은 질문에도 추측하지 말고, "됩니다 — '104477 승인원 표지 생성해줘'처럼 자재번호(여러 개도 가능)와 함께 말씀하시면 SAP에서 자재정보를 조회한 뒤, 출력형식(엑셀/워드/둘 다)·담당자 이름·팀장 이름·가승인원 여부 같은 필요한 항목을 순서대로 여쭤봅니다. 답을 이어서 말씀해주시면(예: '담당자는 홍길동, 팀장은 김철수, 엑셀로, 정식승인원') 항목이 다 모이는 대로 실제 파일을 만들어 C:\\SAP_DMS\\승인원표지\\ 폴더에 저장합니다(Revision 번호와 Remark는 생략하면 기본값이 적용됩니다)"라고 안내하세요(이것도 AI 호출 없이 코드가 여러 턴에 걸쳐 직접 처리하는 별도 명령입니다).
...
위 질문에 대한 답변만 작성하세요. 데이터 값을 담는 JSON 응답은 쓰지 마세요(단, 위 규칙에 따른 [[ACTION:SET_ALARM:번호]] / [[ACTION:CLEAR_ALARM:번호]] / [[ACTION:VIEW_MAIL:번호]] / [[MAIL_DRAFT]]...[[/MAIL_DRAFT]] / [[ACTION:SEND_MAIL:CONFIRM]] / [[ACTION:LOAD_PROJECT:번호]] / [[ACTION:DELETE_ROW:번호]] / [[ACTION:SET_STATUS:번호:상태]] / [[ACTION:TOGGLE_KEY:번호]] / [[ACTION:SET_LEVEL:번호:레벨]] / [[ACTION:MOVE_ROW:번호:방향:칸수]] / [[ACTION:MOVE_ROW_BEFORE:번호:번호]] / [[GANTT_EDIT_DRAFT:번호]]...[[/GANTT_EDIT_DRAFT]] / [[ACTION:APPLY_GANTT_EDIT:CONFIRM]] / [[GANTT_ADD_DRAFT]]...[[/GANTT_ADD_DRAFT]] / [[ACTION:APPLY_GANTT_ADD:CONFIRM]] 태그는 예외입니다). 가독성을 위한 마크다운은 적극 사용하세요: 굵게(**제목**)로 소제목을 달고, "- " 글머리 기호로 항목을 나열하고, 필요하면 그 아래 두 칸 들여쓴 "  - "로 하위 항목(내용/조치 사항 등)을 붙이세요. 여러 이슈를 정리할 때는 이슈별로 소제목(굵게) 하나 + 하위 글머리 기호 여러 개 구조를 기본으로 쓰세요.`;
    ...
    window._buildGanttQaPrompt = async function(question, priorHistory, mailTexts, otherProjectTexts, sapText) {
        ...
        // 💡 [2026-09-14 버그수정] "SAP 조회했는데 AI가 전혀 모르는 척 일반론만 답한다"는 제보의
        //    원인 — 팀이 이 기능 추가 "이전"에 [🤖 AI 문답 → 📝 프롬프트]에서 직접 편집·저장해둔
        //    커스텀 프롬프트(savedTemplate)에는 애초에 "${sapSection}" 토큰이 없다. 그러면 바로 아래
        //    rep()가 그 토큰을 못 찾아 아무 것도 안 하고(split/join은 매치가 없으면 원문 그대로 반환),
        //    실제로 조회해온 SAP 데이터가 프롬프트 어디에도 안 실린 채 조용히 버려진다 — 바로 아래
        //    "🏷️ [필수] 프로젝트 이름 표시 규칙"이 커스텀 프롬프트에도 항상 적용되도록 치환 이후에
        //    고정으로 덧붙이는 것과 정확히 같은 이유로, SAP 데이터도 토큰이 없을 때는 뒤에 강제로
        //    덧붙여 절대 유실되지 않게 한다(토큰이 있는 최신/기본 템플릿에서는 중복을 피하려고
        //    이 강제 첨부를 건너뜀).
        const templateHasSapToken = template.indexOf('${sapSection}') !== -1;
        ...
        // 🐛🐛 [2026-09-17 실사용 재확인] "알람 모두 풀어줘"에 여전히 "일괄 기능이 없다"고
        // 답하는 사고가 프롬프트 템플릿 수정 이후에도 반복됨 — 원인은 line 1262의
        // `localStorage.getItem('gantt_qa_prompt')`: 팀이 [🤖 AI 문답 → 📝 프롬프트]에서
        // 이미 직접 편집·저장해둔 커스텀 프롬프트가 있으면 그게 `window._defaultGanttQaPromptTemplate`
        // (내가 고친 쪽)를 완전히 대체해버려서, 코드에 있는 수정은 이 팀 환경에 전혀 반영되지
        // 않고 있었다 — 바로 위 "🏷️ [필수] 프로젝트 이름 표시 규칙"이 이미 똑같은 이유로
        // "치환 후 무조건 덧붙이기" 방식을 쓰고 있던 것과 동일한 함정. **수정**: 알람 일괄
    ...
    window._renderGanttQaMessages = function() {
        ...
        box.innerHTML = window._ganttQaHistory.map(function(m) {
            ...
            const openExecDraftHtml = (!isUser && m.openExecDraftId && window._ganttQaPendingOpenExecDraft && window._ganttQaPendingOpenExecDraft.id === m.openExecDraftId)
                ? `<div style="display:flex; justify-content:flex-end; gap:6px; margin-top:6px;">
                    <button onclick="window._aiOpenProjectAndReask('${m.openExecDraftId}', this)" onmouseover="this.style.background='#c9ecd3'; this.style.borderColor='#7cc494';" onmouseout="this.style.background='#e6f6ea'; this.style.borderColor='#a8dab8';" style="font-size:11.5px; padding:5px 12px; border:1px solid #a8dab8; background:#e6f6ea; color:#1f7a3d; border-radius:6px; font-weight:bold; cursor:pointer; transition:background .15s, border-color .15s;">${window._currentLang === 'en' ? '🔓 Open project & proceed' : '🔓 프로젝트 열어서 처리'}</button>
