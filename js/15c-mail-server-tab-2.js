    ...
    if (!modal) {
        ...
        <div id="mail-auto-config-box" onclick="event.stopPropagation()" style="pointer-events:all; position:fixed; background:#fff; border-radius:10px; width:var(--modal-w-md); max-width:92vw; max-height:88vh; display:flex; flex-direction:column; box-shadow:0 8px 32px rgba(0,0,0,0.22); top:50%; left:50%; transform:translate(-50%,-50%); resize:both; overflow:hidden; min-width:340px; min-height:300px;">
            <div id="mail-auto-config-drag" style="padding:13px 18px; border-bottom:1px solid #ffe08a; font-weight:bold; font-size:14px; background:#fff8e6; border-radius:10px 10px 0 0; display:flex; justify-content:space-between; align-items:center; cursor:grab; color:#7a5210;">
                <span>⚙️ <span id="mail-auto-config-title">${_en ? 'Mail Auto-Placement Settings' : '메일 자동배치 설정'}</span></span>
            ...
            if (emptyEl) emptyEl.innerHTML =
                `<div style="font-size:32px;">⚠️</div>
                 ...
                 <div style="text-align:left; width:100%; align-self:stretch; box-sizing:border-box; margin-top:14px; padding-top:10px; border-top:1px solid #eee; display:flex; flex-direction:column; flex:1; min-height:0;">
                    ...
                    <button onclick="AR.retryAsManual('${tabKey}', ${idx})" onmouseover="this.style.background='#c9ecd3'; this.style.borderColor='#7cc494';" onmouseout="this.style.background='#e6f6ea'; this.style.borderColor='#a8dab8';" style="margin-top:8px; width:100%; box-sizing:border-box; height:38px; flex-shrink:0; background:#e6f6ea; color:#1f7a3d; border:1px solid #a8dab8; border-radius:6px; font-size:12px; font-weight:bold; cursor:pointer; transition:background .15s, border-color .15s;">✏️ ${_mfEn2 ? 'Register as-is (manual)' : '본문으로 직접 등록'}</button>
                    <button onclick="AR.moveToDirectInput('${tabKey}', ${idx})" onmouseover="this.style.background='#f4d9b3'; this.style.borderColor='#dba354';" onmouseout="this.style.background='#fbead9'; this.style.borderColor='#edbf85';" style="margin-top:4px; width:100%; box-sizing:border-box; height:36px; flex-shrink:0; background:#fbead9; color:#a85d0a; border:1px solid #edbf85; border-radius:6px; font-size:12px; font-weight:bold; cursor:pointer; transition:background .15s, border-color .15s;">🔄 ${_mfEn2 ? 'Move to Direct Input & re-analyze' : '직접 입력 탭으로 이동해서 AI 분석'}</button>
...
window._msBuildProjectMatchSection = function(candidateProjects, mailText, userHint) {
    ...
    return `\n\n--- 프로젝트 매칭 판단 요청 (현재 등록된 활성 프로젝트 전체 목록) ---\n` + userHintBlock +
        ...
        `- 목록 어디에도 해당 안 되거나(신규/미등록 프로젝트) 판단이 애매하면: 0으로 적으세요.\n` +
        `- ⚠️ [복수 프로젝트 — 독립적 용건] 이 메일이 서로 다른 프로젝트 여러 개에 대해 "각각 명확하고 독립적인" 실행 항목(To do)을 담고 있을 때만` +
        ...
        ` 확신이 있을 때만 넣고, 조금이라도 애매하면 절대 넣지 마세요. 그럴 땐 이 배열을 반드시 빈 배열 []로 두세요.\n` +
        `- ⚠️ [복수 프로젝트 — 공통 이슈] 위와 다른 경우로, 이 메일이 "하나의 공통 주제"(같은 제품군의 여러 모델/사이즈에 공통으로 적용되는` +
        ...
        `"복수매칭후보목록": [] (공통 이슈라 여러 프로젝트에 다같이 해당될 후보 번호들, 없으면 반드시 빈 배열 [])`;
};
