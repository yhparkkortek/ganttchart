...
- **여러 자재 문서 일괄 다운로드 — ZDMSR004("DMS 첨부파일 일괄 다운로드 프로그램")
  ...
  - **프런트**: `js/04h`의 `_ganttQaExtractSapBatchDownloadRequest` — 자재번호 2개 이상 +
    "다운로드"/"저장" 키워드로 판정. **반드시 단일 문서 열기 판정
    (`_ganttQaExtractSapOpenDocRequest`)보다 먼저 체크해야 함** — 둘 다 "다운로드"를
    트리거 단어로 인정해서, 순서가 바뀌면 다중 자재 요청이 첫 번째 자재만 처리하는 단일
    열기로 잘못 판정된다(`sendGanttQaMessage`에서 실제로 이 순서로 배치돼 있음).
