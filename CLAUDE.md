...
- **자재번호를 같이 말하면 화면을 미리 열어둘 필요가 없음(2026-09-15 추가)**: 처음엔 "사람이
  ...
  **⚠️⚠️ 실사용 테스트로 확인된 또 다른 함정 — 자재마다 "문서 데이터" 탭이 아예 없을 수
  있음(2026-09-15)**: 자재 106188을 `_navigate_to_material_document_tab`으로 조회했더니
  "뷰 선택" 팝업 없이 곧바로 "기본 데이터"(Basic Data) 탭으로 열렸고, `tabpZU04`("문서
  데이터") 탭 자체가 화면에 없었다 — SAP가 그 자재에 대해 기억하고 있는 "뷰 선택" 이력에
  문서 데이터 뷰가 아예 빠져 있는 것으로 추정(그 자재를 평소 문서 데이터 뷰 없이만
  조회해왔다면 있을 수 있는 일). 이걸 몰랐을 때는 `_select_tab_if_present`가 조용히
  실패해서(탭 없으면 그냥 통과) 엉뚱한 "기본 데이터" 화면 내용을 "문서 데이터 화면"이라고
  잘못 보여주는 버그가 있었다 — 지금은 `_select_tab_with_retry`(재시도 O, 성공여부 반환)로
  바꿔서, 탭을 끝내 못 찾으면 `_navigate_to_material_document_tab`이 명확한 안내 메시지로
  RuntimeError를 낸다("SAP GUI 메뉴의 추가(Extras) → 뷰(Views) → 선택(Select)에서 문서
  데이터를 체크해보라"). **이 상황 자체를 자동으로 해결하는 코드는 아직 없음** — 뷰 선택
  화면/메뉴의 정확한 컨트롤 ID를 모르는 상태라(실사용 녹화 없음), 다음에 이 케이스를
  실제로 겪으면 그 자재로 ① SAP GUI에서 직접 MM03 조회 시 상단 탭 목록에 "문서 데이터"가
  있는지, ②없다면 추가(Extras)→뷰(Views)→선택(Select) 메뉴로 체크했을 때 뜨는 화면을
  녹화해서 정확한 컨트롤을 얻어야 자동화할 수 있다.
