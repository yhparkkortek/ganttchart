...
| `14c-task-inbox.js` | [Phase 1] 업무 보관함 (Task Inbox) — 프로젝트 독립 스테이징. 상단 요약 칩에서 상태→프로젝트를 고르면 그 칩 안에 🗑(`_tiBulkDeleteFiltered`)이 나와 묶음 삭제 — 확인창 없이 지우되 `_tiUndoBuffer`로 [↩ 삭제 취소] 칩 제공(보관함 기록만 삭제 — 간트에 배치된 업무는 그대로). 자동배치 재시도·유휴 스윕(`_ibAutoPlaceSweep`)도 이 파일 |
...
- **Undo 스냅샷은 전체 딥카피가 아니라 구조 공유(증분) 방식**(2026-09-23) — 바뀜지 않은 행은 직전 복사본을 그대로 공유한다(WeakMap 캐시). 그러므로 **스냅샷 복사본은 절대 수정 금지**, 복원할 땐 반드시 새 배열로 복사해서 넣을 것(2000행×40열×50스냅샷 기준 28MB→2.6MB — `docs/gantt-internals.md` Undo 절).
...
- **새 AI-free 로컬 명령(🚫🤖 표시)을 추가하면 그 블록 안에서 직접 `window._ganttQaRecordQuestionFreq(question)`를 호출할 것** — 이 기록은 원래 AI 호출 직전 한 곳에서만 됐는데, AI-free 블록은 그 지점 전에 return해버려 "자주 쓰는 질문"이 안 쌓이는 버그가 있었다(2026-09-23 실사용 확인, BOM/사용처/ZMM009/팀운영비/화면덤프/입고처리/자재내역패턴 전부 해당).
- **"자주 쓰는 질문" 빈도 키는 `_ganttQaPatternKey`(가변 숫자 제외), 재질문 감지는 `_ganttQaNormalizeQ`+`_ganttQaQuestionSimilarity`(숫자/이름이 다르면 다른 질문) — 정반대 요구사항이라 둘을 절대 하나로 합치지 말 것** (2026-09-23: 자재번호가 매번 달라 137건이 전부 count=1로 쌓여 드롭다운이 안 변하던 버그. 4자리 이상 숫자만 치환 — "품목2"/"MRP 2"처럼 의미를 가르는 한두 자리는 남긴다).
- **오래 걸리는 SAP 로컬 명령을 새로 추가해도 "그만"/"중단"으로 자동으로 멈춰지지 않는다** — `POST /sap-cancel`(kortek_backend.py, `_SAP_PROC_STATE`)이 지금 실행 중인 서브프로세스를 kill할 뿐, 각 로컬 명령 블록이 그 kill로 인한 실패 응답을 원래 하던 에러 처리 경로로 자연스럽게 흘려보내는 것뿐이라 별도 배선은 필요 없지만, **새 메시지를 그 자리에 push하는 패턴을 쓰면 안 됨**(`push(pending)...await...pop()+push(final)` 관례가 깨짐) — `docs/sap-lookup.md` "SAP 작업 중단" 절 참고.
- **"자재 여러 건 조회"는 기본이 ZMM009(빠름)** — ZMM009 ALV 컬럼은 **계정별 저장 레이아웃**이라 안 보이는 컬럼(예: 품목2=`ZTEXT`)이 있어도 "그 필드가 없다"가 아니라 "이 계정 레이아웃에 숨겨져 있다"일 수 있다(2026-09-23, 처음엔 없다고 잘못 판단 → 사용자가 레이아웃 변경 화면에서 찾아줌). 그래서 컬럼 존재는 매번 응답 헤더로 실제 확인하고, 없을 때만 느린 MM03 자재별 순회로 폴백 — "이 계정 레이아웃에 그 컬럼을 추가+저장해두면 이후 조회가 코드 변경 없이 빨라진다"는 것도 기억해둘 것.
