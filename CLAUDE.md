...
- **"자재 여러 건 조회"는 기본이 ZMM009(빠름), MM03 자재별 순회(느림)는 정말 그 자재만의 심화 데이터(예: 품목2/"추가 데이터" 서브화면)가 필요할 때만** — ZMM009 그리드에 있다고 착각하고 새 컬럼을 쓰기 전에 라이브로 헤더 행부터 직접 확인할 것(2026-09-23, 없는 컬럼을 있다고 잘못 적어뒀던 사고).
