...
- **엑셀 내보내기**: "엑셀로 저장해줘"/"다운로드해줘"류 명령은 `_ganttQaTryHandleSapExportCommand`(04h)가
  AI 호출 없이 로컬에서 바로 가로채, 방금 조회 성공한 `window._lastSapFetchResult`(원본 탭 구분
  텍스트)를 `_exportSapDataToExcel`로 그대로 XLSX 변환한다 — **AI 답변 텍스트가 아니라 원본을 쓰는
  이유**: AI가 요약하면서 빈 항목을 `` 같은 빈 코드블록으로 얼버무리는 등 값이 미묘하게 바뀌는
  사례가 실사용에서 확인됐음. "SAP 데이터 있어?/가능해?" 같은 **질문형**은 물음표나 "가능/되나/될까"
...
- **ZMM009("자재 List(복수조회)") 등 다중 자재 리스트 화면에서 바로 문서 열기(2026-09-15
  ...
  우선순위가 낮아짐).
- **ZDMSR004("DMS 첨부파일 일괄 다운로드 프로그램") 발견 — 진짜 배치 다운로드는 이걸로
  (2026-09-15, 아직 미구현)**: 두 번째 화면 녹화(ScreenSketch 일반 녹화, SAP GUI 매크로
  아님)에서 확인 — MM03 드릴다운을 한 건씩 반복하는 대신, 이미 회사 SAP에 "여러 자재의
  문서를 한 번에 다운로드하는" 전용 커스텀 리포트가 있었다. 흐름: ① `ZDMSR004` 실행 →
  ② "자재코드" 필드의 "복수 선택" 버튼으로 여러 자재번호를 한꺼번에 입력(엑셀에서 복사한
  값을 표준 SAP "복수 선택" 팝업에 붙여넣기 — ZMM009의 "자재 복수 선택" 팝업과 동일한
  SAP 표준 컨트롤로 추정됨, 함수그룹 SAPLALDB 기반) → ③ 실행(F8) → 그 자재들의 모든
  문서(P01 등)가 ALV 그리드 리스트로 한 번에 뜸(자재코드/문서유형/전표번호/파일이름/내역
  컬럼) → ④ "다운로드" 버튼 클릭 → `C:\SAP_DMS\<문서번호>\<원본파일명>` 구조로 **전부
  로컬에 자동 다운로드됨**(문서번호별 하위폴더 자동 생성, 실사용 화면에서 확인). 이게
  사용자가 원한 "복수 조회 + 자료 열기 + 저장"을 SAP 자체 기능으로 한 번에 해결하는 진짜
  정답 — MM03 드릴다운 반복보다 훨씬 낫다. **미구현 이유**: 자재코드 복수 선택 팝업과
  다운로드 버튼의 정확한 컨트롤 ID를 아직 모름(일반 화면 녹화라 컨트롤 ID가 안 잡힘) +
  "다운로드" 버튼이 여는 폴더 선택 창이 SAP GUI 객체 트리 밖의 **네이티브 Windows
  다이얼로그**일 가능성이 높아(앞서 "다른 이름으로 저장" 엑셀 내보내기 때와 동일한 패턴)
  SAP GUI Scripting만으로는 못 건드릴 수 있음 — 다음에 이 기능을 만들 때는 SAP GUI
  "기록 및 재생" 매크로로 ①~④ 전 과정을 다시 녹화해서 정확한 ID를 확보할 것(요청함,
  아직 응답 대기 중).
