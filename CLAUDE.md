...
- **검증**: 브라우저에서 `fetch`/`callAiBackend`를 모킹해 전체 왕복(PDF 첨부 → AI 추출 →
  ...
  왕복 테스트로 검증함. **SAP GUI 자동화 자체(ZMMR060 업로드~ZMM018 출력)는 아직 실제
  SAP 세션에서 라이브 검증 전** — 사용된 필드 ID 중 "플랜트"/"파일" 두 개는 추측(방어적
  실패 처리 있음), 나머지는 사용자 제공 매크로 기반 정확한 값이지만 실제 실행은 처음이라
  실사용에서 문제가 생기면 이 절의 각 항목부터 의심할 것.
