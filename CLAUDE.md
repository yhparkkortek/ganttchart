...
- **🐛🐛 [2026-09-17 실사용 재확인] 위 수정도 "SAP/BOM/역전개/사용처" 키워드가 있을 때만
  ...
  - **⚠️⚠️ 이 수정으로도 "ZMM009로 자동 라우팅"은 아직 안 됨 — 별개의 미해결 기능 요청**:
    `_aiFetchSapContext`는 BOM/사용처/역전개가 아닌 나머지 모든 경우(자재번호만 있는 순수
    "조회/출력" 요청 포함) 마지막 `else` 분기(`url = '/sap-fetch'`)로 빠지는데, 이 범용
    엔드포인트는 **자재번호를 전혀 받지 않고 그 순간 SAP GUI 화면에 떠 있는 걸 그대로 읽어올
    뿐**이다 — 즉 지금은 "자재번호 목록 + 조회/출력" 요청에 대해 ZMM009든 MM03이든 **자동으로
    항해하는 로직 자체가 아예 없다**(사람이 미리 원하는 화면을 SAP GUI에서 열어둬야 함, 기존
    "SAP 조회" 설계 원칙 그대로). 사용자가 "ZMM009를 기본으로 쓰면 좋겠다"고 요청했으나,
    이 저장소의 다른 모든 SAP 화면 자동화와 마찬가지로 **ZMM009 초기화면의 정확한 필드 ID
    (플랜트/판매조직/유통경로 3개 필수 입력 + 자재 "복수 선택" 팝업의 정확한 버튼 시퀀스 —
    ZDMSR004/ZPP038/ZMM046처럼 트랜잭션마다 버튼 횟수가 다를 수 있음이 이미 여러 번 확인된
    바 있음)를 **추측하지 않고** 라이브 진단이나 SAP GUI "기록 및 재생" 매크로로 먼저
    확보해야 안전하게 구현할 수 있다 — 다음에 이 기능을 다시 요청받으면 매크로/라이브
    진단부터 요청할 것(추측성 필드 ID로 구현하면 조용히 엉뚱한 자재를 조회하거나 SAP 필수
    입력 오류로 막히는 사고가 이 저장소에서 여러 번 재현된 패턴이라 특히 위험함).
