...
- **✅ [2026-09-15 실사용 SAP 세션으로 전 구간 라이브 검증 완료]** — 사용자가 실제로 SAP를
  ...
  4. **`print_po_via_zmm018` 신규 분리 + 라이브 검증 완료**: 원래 `confirm_save_po`
     ...
     수 있어 재확인이 필요함).
  5. **결론**: 이 기능의 SAP GUI 자동화 파이프라인은 엑셀 업로드부터 발주서 출력까지
     전 구간이 실제 SAP 세션에서 동작함을 확인했다 — 이 절 위쪽의 "필드 ID 추측"
     경고들은 이제 "검증됨"으로 갱신 가능하지만, 이 테스트는 자재 1건짜리 단일 케이스라
     복수 품목(여러 행)에서 협력사 F4 검색을 행마다 반복해야 하는지 여부(현재는 한 번만
     선택) 등은 여전히 미검증 상태로 남아있다.
