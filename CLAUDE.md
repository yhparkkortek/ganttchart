...
- **✅ [2026-09-15 실사용 SAP 세션으로 전 구간 라이브 검증 완료]** — 사용자가 실제로 SAP를
  ...
  6. **해결 — SAP GUI Scripting이 아닌 별도 계층(Windows UI Automation, `pywinauto`)
     ...
     **⚠️⚠️ 이 함수는 Claude Code 세션(harness)이 직접 라이브 테스트를 못 한다** —
     ...
     - **1차 실사용 테스트 실패(2026-09-15)**: "PDF 저장(사본 저장) 다이얼로그가 뜨지
       않았습니다" 오류 — Ctrl+S를 SAP 창에 보냈지만, "발주서출력" 버튼을 누른 직후라
       SAP GUI 자체의 키보드 포커스가 여전히 그 버튼(또는 결과 그리드)에 남아있어서
       임베드된 PDF 뷰어로 단축키가 전달되지 않은 것으로 추정. **수정**: 단축키를
       보내기 전에 SAP 창 중앙(문서 내용 영역으로 추정되는 위치)을 한 번 클릭해 포커스를
       명시적으로 옮기도록 보강. 또한 사용자가 "Ctrl+Shift+S 아니냐"고 제안 — 임베드
       뷰어가 Acrobat 계열이면 "저장"과 "다른 이름으로 저장"을 구분해서 실제로는
       Ctrl+Shift+S가 필요할 수 있어, **Ctrl+S를 먼저 시도하고 다이얼로그가 안 뜨면
       Ctrl+Shift+S로 자동 재시도**하도록 변경. 실패 시 에러 메시지에 그 시점에
       열려있던 창 제목 목록을 같이 남겨(`_find_save_dialog`), 다음에 또 실패해도
       왕복 없이 바로 원인을 좁힐 수 있게 함. **이 수정도 아직 라이브 미검증** — 다음
       테스트에서 여전히 실패하면 클릭 좌표(문서 영역이 아닌 다른 곳을 클릭했을
       가능성)나 두 단축키 모두 이 뷰어에 안 맞을 가능성을 의심할 것 — 그 경우
       마지막 수단으로 픽셀 좌표 기반 저장 아이콘 직접 클릭을 고려해야 함(화면
       녹화로 확인된 위치: 미리보기 하단 중앙 툴바의 💾 아이콘).
  7. **결론**: 엑셀 업로드~협력사 검색~단가 입력~저장~오더번호 확보~ZMM018 조회까지는
     실제 SAP 세션에서 라이브 검증 완료(위 1~4번). **PDF를 실제 파일로 저장하고 여는
     마지막 단계(5~6번)는 코드는 작성했지만 위 harness 제약으로 아직 라이브 검증 전** —
     다음 실사용 테스트에서 반드시 이 부분부터 확인할 것. 그 외에도 이 테스트는 자재
     1건짜리 단일 케이스라, 복수 품목(여러 행)에서 협력사 F4 검색을 행마다 반복해야
     하는지 여부(현재는 한 번만 선택) 등은 여전히 미검증 상태로 남아있다.
