...
- **AI 추출**: `window._ganttQaExtractPoItemsViaAi(apiKey, attachments, correctionNote)` —
  ...
  - **[2026-09-15 신규] 같은 품목 목록이 문서 안에 중복 등장하는 경우 대비** — 거래명세서
    페이지와 견적서(QUOTATION) 페이지가 같은 품목들을 각자의 표 형식으로 중복 기재하는
    실제 문서(사용자 제공 예시, 2페이지짜리 PDF)가 확인되어, "같은 품목이 문서 안에
    두 번 이상 나올 수 있다 — 품번/품명으로 같은 품목인지 판단해 한 번만 셀 것"을
    프롬프트에 추가함(아직 실사용 검증 전 — AI가 실제로 잘 걸러내는지는 다음 테스트에서
    확인 필요).
...
- **SAP 자동화(ZMMR060 업로드 → 협력사/세금코드/단가)**: `sap_bridge_32.py`의
  ...
  F4 팝업 왕복이 늘어나 시간이 더 걸림 — 타임아웃 계산 시 감안할 것). ⚠️⚠️ **EPEIN을
