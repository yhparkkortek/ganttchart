...
- **✅ [2026-09-15 실사용 SAP 세션으로 전 구간 라이브 검증 완료]** — 사용자가 실제로 SAP를
  ...
  6. **해결 — SAP GUI Scripting이 아닌 별도 계층(Windows UI Automation, `pywinauto`)
     ...
     **⚠️⚠️ 이 함수는 아직 라이브 검증 전이다** — Claude Code 세션이 Bash로 이 함수를
     직접 실행해보려 하자, "다른 창에 키보드 입력을 보내는" 동작(SAP GUI Scripting COM
     호출보다 훨씬 넓은 범위의 일반 OS 자동화라서)을 harness의 자동 모드 분류기가
     "[Auto-Mode Bypass]"로 차단함 — 대화창 승인과 무관한 harness 레벨 안전장치라
     우회 시도하지 않았다(SAP 저장 자체가 막혔던 것과 같은 종류의 제약, 다만 이번엔
     차단 사유가 "재무적 커밋"이 아니라 "임의 창에 대한 OS 수준 입력 자동화"라는 점이
     다름). **이 차단은 Claude Code 세션이 Bash로 직접 실행할 때만 해당** — 실제 배포된
     앱(브라우저 → `kortek_backend.py` → 이 서브프로세스) 경로는 Claude Code Bash를
     거치지 않으므로 이 차단과 무관하다. 문제가 생기면 이 함수부터 의심하고, 디버깅이
     필요하면 **사용자가 직접** 자기 터미널에서 `py -3-32 sap_bridge_32.py
     print_po_via_zmm018 <오더번호>`를 실행해 재현할 것(harness 제약이 없는 경로).
