...
- **ZMM009("자재 List(복수조회)") 등 다중 자재 리스트 화면에서 바로 문서 열기(2026-09-15
  ...
  단일 자재 직접조회로 동작하는 하위호환 설계. **아직 없는 것**: 여러 자재의 문서를 한 번의
  명령으로 전부 여는 배치 기능(예: "이 목록 자재들 문서 다 열어줘") — 한 번에 하나씩,
  채팅으로 반복 요청하는 방식만 지원. 배치로 만들려면 문서를 연 뒤 리스트로 "뒤로가기"하는
  정확한 컨트롤을 아직 몰라(녹화 없음) 추가 확인이 필요함.
